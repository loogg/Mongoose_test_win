# Instructions: 设备控制台应用开发规范

本文档定义了项目的开发规范，在每次对话中都应遵循这些约定。
...
**核心思路**：
- 使用 `useState` 管理侧边栏的显示/隐藏状态
- 使用 `useEffect` 监听窗口大小变化，自动调整侧边栏状态
- 使用 Tailwind CSS 的 `block` 和 `hidden` 类控制侧边栏的显示/隐藏
- 使用 `flex` 布局确保内容区自动调整宽度

### 22.2 最佳实践

**状态管理**：
- 使用 `useState(window.innerWidth >= 768)` 初始化侧边栏状态，确保页面加载时根据屏幕宽度自动调整
- 使用 `useEffect` 监听窗口大小变化，实现响应式布局

**布局优化**：
- 使用 `flex` 布局确保内容区自动调整宽度
- 使用 `flex-1` 确保导航区域可滚动，同时用户信息区域始终在底部
- 使用 `transition-all duration-300 ease-in-out` 添加平滑的过渡动画效果

**用户体验**：
- 在移动设备上点击导航后自动关闭侧边栏，提升用户体验
- 菜单按钮在所有屏幕尺寸下都可见，确保用户随时可以访问导航功能
- 用户信息区域始终在侧边栏底部，不会因屏幕宽度变化而改变位置

**性能优化**：
- 使用 `useCallback` 优化事件处理函数（如果需要）
- 避免不必要的重新渲染
